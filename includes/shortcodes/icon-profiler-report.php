...
if ( ! function_exists( 'icon_profiler_report_render' ) ) {
	...
	function icon_profiler_report_render( $atts ) {
		...
		$export_mode = isset( $_GET['icon_psy_export'] ) ? sanitize_key( wp_unslash( $_GET['icon_psy_export'] ) ) : '';
		$is_pdf      = ( $export_mode === 'pdf' );
		...
		// ── PDF image helpers ──
		if ( ! function_exists( 'icon_psy_bytes_look_like_image' ) ) {
			function icon_psy_bytes_look_like_image( $bytes ) {
				if ( ! is_string( $bytes ) || strlen( $bytes ) < 12 ) return false;
				$h = substr( $bytes, 0, 12 );
				if ( substr( $h, 0, 8 ) === "\x89PNG\x0D\x0A\x1A\x0A" ) return true;
				if ( substr( $h, 0, 3 ) === "\xFF\xD8\xFF" ) return true;
				if ( substr( $h, 0, 3 ) === "GIF" ) return true;
				if ( substr( $h, 0, 4 ) === "RIFF" && substr( $h, 8, 4 ) === "WEBP" ) return true;
				$h2 = strtolower( substr( ltrim( $bytes ), 0, 120 ) );
				return strpos( $h2, '<svg' ) !== false;
			}
		}
		if ( ! function_exists( 'icon_psy_guess_mime_from_ext' ) ) {
			function icon_psy_guess_mime_from_ext( $path ) {
				$map = array('png'=>'image/png','jpg'=>'image/jpeg','jpeg'=>'image/jpeg','gif'=>'image/gif','svg'=>'image/svg+xml','webp'=>'image/webp');
				$ext = strtolower( pathinfo( (string) $path, PATHINFO_EXTENSION ) );
				return isset( $map[$ext] ) ? $map[$ext] : 'image/png';
			}
		}
		if ( ! function_exists( 'icon_psy_url_to_local_upload_path' ) ) {
			function icon_psy_url_to_local_upload_path( $url ) {
				$url = (string) $url;
				if ( $url === '' || ! function_exists( 'wp_upload_dir' ) ) return '';
				...
				return $rel === '' ? '' : $basedir . '/' . $rel;
			}
		}
		if ( ! function_exists( 'icon_psy_local_file_to_data_uri' ) ) {
			function icon_psy_local_file_to_data_uri( $path ) {
				$path = (string) $path;
				if ( $path === '' || ! file_exists( $path ) || ! is_readable( $path ) ) return '';
				$bytes = @file_get_contents( $path );
				if ( empty( $bytes ) || ! icon_psy_bytes_look_like_image( $bytes ) ) return '';
				return 'data:' . icon_psy_guess_mime_from_ext( $path ) . ';base64,' . base64_encode( $bytes );
			}
		}
		if ( ! function_exists( 'icon_psy_fetch_image_data_uri' ) ) {
			function icon_psy_fetch_image_data_uri( $url ) {
				$url = trim( (string) $url );
				if ( $url === '' ) return '';
				$local = icon_psy_url_to_local_upload_path( $url );
				if ( $local ) { $du = icon_psy_local_file_to_data_uri( $local ); if ( $du ) return $du; }
				if ( function_exists( 'attachment_url_to_postid' ) && function_exists( 'get_attached_file' ) ) {
					$aid = (int) attachment_url_to_postid( $url );
					if ( $aid > 0 ) { $f = (string) get_attached_file( $aid ); if ( $f ) { $du = icon_psy_local_file_to_data_uri( $f ); if ( $du ) return $du; } }
				}
				$resp = wp_remote_get( $url, array('timeout'=>25,'sslverify'=>true,'headers'=>array('Accept'=>'image/*,*/*;q=0.5')) );
				if ( is_wp_error( $resp ) ) return '';
				if ( (int) wp_remote_retrieve_response_code( $resp ) !== 200 ) return '';
				$body = wp_remote_retrieve_body( $resp );
				if ( empty( $body ) || ! icon_psy_bytes_look_like_image( $body ) ) return '';
				$ct = wp_remote_retrieve_header( $resp, 'content-type' );
				$ct = is_string( $ct ) ? strtolower( $ct ) : '';
				if ( $ct === '' || strpos( $ct, 'image/' ) === false ) $ct = icon_psy_guess_mime_from_ext( $url );
				return 'data:' . $ct . ';base64,' . base64_encode( $body );
			}
		}
		if ( ! function_exists( 'icon_profiler_img_src' ) ) {
			function icon_profiler_img_src( $url, $is_pdf ) {
				$url = trim( (string) $url );
				if ( $url === '' ) return '';
				if ( $is_pdf ) { $data = icon_psy_fetch_image_data_uri( $url ); return $data ? $data : ''; }
				return $url;
			}
		}

		// ── PNG map for PDF ──
		if ( ! function_exists( 'icon_profiler_make_map_png_data_uri' ) ) {
			function icon_profiler_make_map_png_data_uri( $x_n, $y_n, $w = 980, $h = 640 ) {
				if ( ! function_exists( 'imagecreatetruecolor' ) ) return '';
				$im = imagecreatetruecolor( $w, $h );
				if ( ! $im ) return '';
				imagealphablending( $im, true ); imagesavealpha( $im, true );
				$white = imagecolorallocate( $im, 255,255,255 ); $line  = imagecolorallocate( $im, 190,200,210 );
				$txt   = imagecolorallocate( $im, 10,40,35 );    $muted = imagecolorallocate( $im, 92,110,105 );
				$q1 = imagecolorallocate($im,232,248,255); $q2 = imagecolorallocate($im,233,248,241);
				$q3 = imagecolorallocate($im,242,252,246); $q4 = imagecolorallocate($im,240,248,255);
				imagefilledrectangle( $im, 0, 0, $w-1, $h-1, $white );
				$pad = (int) round( min($w,$h) * 0.11 );
				$cx  = (int) round( $w/2 );
				$cy  = (int) round( $h/2 );
				imagefilledrectangle($im,$pad,$pad,$cx,$cy,$q1);   imagefilledrectangle($im,$cx,$pad,$w-$pad,$cy,$q2);
				imagefilledrectangle($im,$pad,$cy,$cx,$h-$pad,$q3); imagefilledrectangle($im,$cx,$cy,$w-$pad,$h-$pad,$q4);
				imagerectangle($im,$pad,$pad,$w-$pad,$h-$pad,$line);
				imageline($im,$cx,$pad,$cx,$h-$pad,$line); imageline($im,$pad,$cy,$w-$pad,$cy,$line);
				imagestring($im,5,$pad+10,$cy-22,'People',$txt);  imagestring($im,5,$w-$pad-64,$cy-22,'Task',$txt);
				imagestring($im,5,$cx+10,$pad+10,'Pace',$txt);    imagestring($im,5,$cx+10,$h-$pad-24,'Steady',$txt);
				imagestring($im,4,$pad+10,$pad+10,'Connector',$muted); imagestring($im,4,$w-$pad-82,$pad+10,'Driver',$muted);
				imagestring($im,4,$pad+10,$h-$pad-20,'Supporter',$muted); imagestring($im,4,$w-$pad-78,$h-$pad-20,'Thinker',$muted);
				$x_n = max(-1.0,min(1.0,(float)$x_n)); $y_n = max(-1.0,min(1.0,(float)$y_n));
				$rx  = ($w-2*$pad)/2; $ry = ($h-2*$pad)/2;
				$dx  = (int) round($cx+($x_n*$rx)); $dy = (int) round($cy-($y_n*$ry));
				$glow = imagecolorallocatealpha($im,20,164,207,90);
				imagefilledellipse($im,$dx,$dy,56,56,$glow);
				imagefilledellipse($im,$dx,$dy,24,24,imagecolorallocate($im,15,118,110));
				ob_start(); imagepng($im); $png = ob_get_clean(); imagedestroy($im);
				return empty($png) ? '' : 'data:image/png;base64,'.base64_encode($png);
			}
		}
		...
		$map_png_data_uri = $is_pdf ? icon_profiler_make_map_png_data_uri($x_n,$y_n) : '';
