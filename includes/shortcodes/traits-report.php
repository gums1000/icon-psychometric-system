 ...
 * Usage (PDF):    /traits-report/?participant_id=XX&icon_psy_export=pdf  (handled by central PDF engine)
 *
 * This file is now SCREEN-ONLY:
 * - ✅ Keeps the full on-screen report (HTML + Chart.js radar)
 * - ✅ Keeps the Export PDF button
 * - ✅ Removes ALL Dompdf/PDF cover/header/footer/PDF CSS/radar PNG generation
 * - ✅ On PDF request, hands off to a central engine (so this file stays lean)
 *
 * UPDATE (this request):
 ...
 *   - Allows resolve participant_id from ?share_token=XXXX (screen + pdf)
...
if ( ! function_exists( 'icon_psy_traits_report' ) ) {
	...
	function icon_psy_traits_report( $atts ) {
		...
		// ------------------------------------------------------------
		// PDF handoff (central engine) — DO THIS FIRST
		// ------------------------------------------------------------
		...
		if ( $export_mode === 'pdf' ) {
			...
			// allow token -> participant in pdf mode too
			if ( $participant_id <= 0 && $token !== '' && $raters_table_exists ) {
				$rr = $wpdb->get_row(
					$wpdb->prepare(
						"SELECT participant_id FROM {$raters_table} WHERE token = %s LIMIT 1",
						$token
					)
				);
				if ( $rr && ! empty( $rr->participant_id ) ) {
					$participant_id = (int) $rr->participant_id;
				}
			}

			// allow share_token -> participant in pdf mode too
			$share_token_pdf = isset( $_GET['share_token'] ) ? sanitize_text_field( wp_unslash( $_GET['share_token'] ) ) : '';
			$share_token_pdf = preg_replace( '/[^a-zA-Z0-9\-_]/', '', (string) $share_token_pdf );
			if ( $participant_id <= 0 && $share_token_pdf !== '' ) {
				$pid_from_share = icon_psy_traits_share_resolve_participant_id( $share_token_pdf );
				if ( $pid_from_share > 0 ) $participant_id = (int) $pid_from_share;
			}

			if ( $participant_id <= 0 ) {
				wp_die( 'Missing participant_id.' );
			}

			// ✅ IMPORTANT: set ONE real path here once you decide it
			// (replace this with your actual path)
			$engine_file = dirname( __FILE__, 3 ) . '/includes/pdf/icon-psy-pdf-engine.php';

			if ( ! file_exists( $engine_file ) ) {
				wp_die( 'PDF engine file not found at: ' . esc_html( $engine_file ) );
			}

			require_once $engine_file;

			if ( ! function_exists( 'icon_psy_pdf_render_and_stream' ) ) {
				wp_die( 'PDF engine loaded but icon_psy_pdf_render_and_stream() is missing.' );
			}

			icon_psy_pdf_render_and_stream( 'traits', array(
				'participant_id' => $participant_id,
			) );
