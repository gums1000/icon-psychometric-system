...
if ( ! function_exists( 'icon_psy_lens_strengthen_play' ) ) {
	function icon_psy_lens_strengthen_play( $weak_lens, $band, $competency, $type = 'general' ) {
		...
		// Base moves by weakest lens (existing)
		$everyday = array(
		...
		// For high band: scale and teach, not “fix”
		if ( $band === 'high' ) {
			$scale = array(
...
if ( ! function_exists( 'icon_psy_lens_impact_sentence' ) ) {
	function icon_psy_lens_impact_sentence( $band, $type, $key_seed ) {
		...
		// Tweak by band so it doesn’t sound the same
		if ( $band === 'low' ) {
			$set = array(
...
if ( ! function_exists( 'icon_psy_lens_templates' ) ) {
	function icon_psy_lens_templates() {
		...
		// Helper to build a standard two-paragraph narrative with higher variety
		$build = function( $ctx, $code, $theme_key, $default_comp_label ) {
			...
			// Add pattern, gap calibration, signal confidence, and type-based impact
			...
			// Paragraph 2: strengthening guidance (weakest lens + band + type)
			$p2 = icon_psy_lens_strengthen_play( $pat['weak_lens'], $band, $comp, $type );

			// Add a short “close” line, varied and type-shaped
			$close_map = array(
				'communication' => array(
...
if ( ! function_exists( 'icon_psy_lens_narrative_text' ) ) {
	function icon_psy_lens_narrative_text( $ctx ) {
		...
		// Normalise expected keys (backwards compatible)
		...
		if ( isset( $templates[ $lens ] ) && is_callable( $templates[ $lens ] ) ) {
			...
			// Generic fallback (still two paragraphs, still varied by competency type)
		...
		// Guarantee two paragraphs always
...
if ( ! function_exists( 'icon_psy_lens_narrative_html' ) ) {
	function icon_psy_lens_narrative_html( $ctx ) {
		...
		$html  = '<p style="margin:0 0 8px;font-size:12px;color:#4b5563;line-height:1.55;">' . esc_html( $p1 ) . '</p>';
		$html .= '<p style="margin:0 0 8px;font-size:12px;color:#4b5563;line-height:1.55;">' . esc_html( $p2 ) . '</p>';
